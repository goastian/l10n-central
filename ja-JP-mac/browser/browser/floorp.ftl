...
floorp-home-mode-choice-default = 
    .label = { -brand-short-name } Home (デフォルト)
...
Gesturefy = Gesturefy
about-Gesturefy = Gesturefy はブラウザーにマウスジェスチャーを追加する拡張機能です。{ -brand-short-name } がこのアドオンのインストールを検出すると、{ -brand-short-name } 限定のジェスチャーコマンドを Gesturefy に追加します。また、このアドオンは新規タブでも動作します。
# Translate
...
floorp-update-application-auto-enabled-option = 
    .label = { -brand-short-name } Portable のアップデートを自動的に確認します。(推奨)
...
memo-input-placeholder = ここにメモを入力またはペースト
delete-memo = 削除
save-memo = 保存
...
welcome-describe-browser-design = サードパーティによる素晴らしいデザインから { -brand-short-name } の外観を選択できます。OS 固有のデザインも設定で使用可能です。
...
floorp-custom-actions-close-tab = 現在のタブを閉じる
    .label = 現在のタブを閉じる
floorp-custom-actions-open-new-window = 新規ウインドウを開く
    .label = 新規ウインドウを開く
floorp-custom-actions-open-new-private-window = 新規プライベートウインドウを開く
    .label = 新規プライベートウインドウを開く
...
floorp-custom-actions-search-the-web = ウェブを検索
    .label = ウェブを検索
floorp-custom-actions-open-migration-wizard = 移行アシスタントを開く
    .label = 移行アシスタントを開く
...
floorp-open-profile-with-new-instance = 起動
    .tooltiptext = このプロファイルで { -brand-short-name } を起動
...
split-view-position-left = デフォルト
...
floorp-disable-toolbars-description = ウェブアプリのウインドウですべての拡張機能とツールバーが非表示になります。
