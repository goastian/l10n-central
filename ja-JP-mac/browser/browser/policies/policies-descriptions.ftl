...
policy-DontCheckDefaultBrowser = 起動時にデフォルトウェブブラウザーを確認する機能を無効にします。
...
policy-LegacySameSiteCookieBehaviorEnabled = 従来の SameSite Cookie の動作設定をデフォルトで有効にします。
...
policy-PrintingEnabled = プリント機能を有効または無効にします。
...
policy-StartDownloadsInTempDirectory = デフォルトダウンロードディレクトリーではなく、ローカルの一時的な場所にダウンロードするよう強制します。
...
policy-UseSystemPrintDialog = システムのプリントダイアログでプリントします。
